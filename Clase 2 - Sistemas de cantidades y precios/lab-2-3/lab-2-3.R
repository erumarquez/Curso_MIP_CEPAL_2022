...
##### ejercicio de clase 2: Ante un aumento homogéneo entre sectores del 1% en la demanda final: ¿cuánto crece el producto bruto?


f_3 <- f # Nueva demanda final
f_3 <- f_3*1.01 # Nueva demanda final

x_4 <- B%*%f_3 # Nuevo producto bruto (resolviendo el sistema de gasto)
x_var_4 <- x_4/x-1 # Vector con las variaciones sector por sector
x_var_4[is.nan(x_var_4)] <- 0
x_var_tot2 <- (colSums(x_4)/colSums(x)-1)*100 # Variaci?n del VBP total

res_ej_4 <- rbind(cbind(tbl_ind['desc_short'],x_var*100),c('Total',x_var_tot))

colnames(res_ej_4) <- c("desc","Var %")
rownames(res_ej_4) <- c(rownames(Z),"Total")

write.csv(res_ej_4,results_dir%+%"res_ej_4.csv")


########## ) Si aumenta 1% el consumo de los hogares, ¿cuánto crece el producto? ¿Y si lo que aumenta es la inversión? ¿Y las exportaciones? Comparar. ------------

# rm(list=ls())
# 
# fh <- F[1:45]
# 
# fh <- matrix(fh,nrow=45,ncol=1) 

## aca hay que remplazar en la estructura de codigo siguiente a f_h por fh que creamos arriba --


# f_h <- F # Nueva demanda final
# f_2[1] <- f_2[1]*1.1 # Nueva demanda final
# x_2 <- B%*%f_2 # Nuevo producto bruto (resolviendo el sistema de gasto)
# x_var <- x_2/x-1 # Vector con las variaciones sector por sector
# x_var[is.nan(x_var)] <- 0
...
#### --- Calcular el impacto en el sistema de precios relativos de una duplicación de los salarios y los beneficios en el sector de combustibles (s10_19) ------- 
